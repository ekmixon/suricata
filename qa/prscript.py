...
docker_deps = "" if GOT_DOCKER else " (disabled)"
...
parser.add_argument(
    ...
    action='store_const',
    const=True,
    help=f'create docker container{docker_deps}',
    default=False,
)
...
parser.add_argument(
    ...
    action='store_const',
    const=True,
    help=f'start docker container{docker_deps}',
    default=False,
)
...
parser.add_argument(
    ...
    action='store_const',
    const=True,
    help=f'stop docker container{docker_deps}',
    default=False,
)
...
parser.add_argument(
    ...
    action='store_const',
    const=True,
    help=f'remove docker container and image{docker_deps}',
    default=False,
)
...
if args.docker:
    ...
    BUILDERS_LIST = [username, f"{username}-pcap"]

BUILDERS_URI = f"{BASE_URI}builders/"
JSON_BUILDERS_URI = f"{BASE_URI}json/builders/"

if GOT_NOTIFY and args.notify:
...
def TestGithubSync(branch):
    ...
    return -1 if sha_local != sha_github else 0
...
def OpenBuildbotSession():
    ...
    request = urllib.request.Request(f'{BASE_URI}login', params)
...
def SubmitBuild(branch, extension = "", builder_name = None):
    ...
    if args.docker:
        if "<h2>Pending Build Requests:</h2>" not in result:
            return -1
        print("Build '" + builder_name + "' submitted")
        return 0
    if b'Current Builds' not in result:
        return -1
    print("Build '" + builder_name + "' submitted")
    return 0
...
def FindBuild(branch, extension = "", builder_name = None):
    ...
    return json_result["cachedBuilds"][-1] if json_result["cachedBuilds"] else -2
...
def GetBuildStatus(builder, buildid, extension="", builder_name = None):
    ...
    if json_result["currentStep"]:
        return 1
    return 0 if 'successful' in json_result["text"] else -1
...
def WaitForBuildResult(builder, buildid, extension="", builder_name = None):
    ...
    # return the result
    if res == 0:
        print(f"Build successful for {builder_name}")
    else:
        print(
            f"Build failure for {builder_name}: {BUILDERS_URI}{builder_name}/builds/{str(buildid)}"
        )
...
if not args.local:
    ...
    if ret != 0:
        if ret == -2:
            print(f"Branch {args.branch} is not pushed to Github.")
            sys.exit(-1)
        if args.norebase:
            print(
                f"Branch {args.branch} is not in sync with corresponding Github branch. Continuing due to --norebase option."
            )

        else:
            print(
                f"Branch {args.branch} is not in sync with corresponding Github branch. Push may be needed."
            )
    ...
    if TestRepoSync(args.branch) != 0:
        if args.norebase:
            print(
                ...
                + " is not in sync with OISF's master branch. Continuing due to --norebase option."
            )

        else:
            print(
                f"Branch {args.branch}"
                + " is not in sync with OISF's master branch. Rebase needed."
            )
...
def CreateContainer():
    ...
    if GOT_DOCKERPY_API < 2:
        ...
        print(f"Using base src dir: {suri_src_dir}")
...
for build in BUILDERS_LIST:
    ...
    if buildid == -1:
        print(
            f"Pending build tracking is not supported. Follow build by browsing {BUILDERS_URI}{build}"
        )

    elif buildid == -2:
        print(f"No build found for {BUILDERS_URI}{build}")
        sys.exit(0)
    else:
        if not args.docker:
            print(
                f"You can watch build progress at {BUILDERS_URI}{build}/builds/{str(buildid)}"
            )

        buildids[build] = buildid

if args.docker:
    print(f"You can watch build progress at {BASE_URI}waterfall")
...
if args.docker:
    while len(buildids):
        ...
        for build, value in buildids.items():
            ...
            if ret == -1:
                ...
                print(
                    f"Build failure for {build}: {BUILDERS_URI}{build}/builds/{str(value)}{remains}"
                )
                ...
                if len(up_buildids):
                    ...
                    remains = ""
                print(f"Build successful for {build}{remains}")
        time.sleep(5)
...
if buildres == 0:
    if not args.norebase and not args.docker:
        ...
        for build, value_ in buildids.items():
            print(f"- PR {build}: {BUILDERS_URI}{build}/builds/{str(value_)}")
